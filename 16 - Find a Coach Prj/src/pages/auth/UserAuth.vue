<template>
  <div>
    <BaseDialog :show="!!error" @close="handleErr"
        title="An  error occurred." fixed>
      <p>{{ error }}</p>
    </BaseDialog>
    <BaseDialog :show="isLoading"
        title="Authenticating..." fixed>
      <BaseSpinner />
    </BaseDialog>
    <BaseCard>
  ...
  methods: {
    async submitForm() {
      ...
      try {
        ...
        this.error = err.message || 'Failed, try later. '
      }
